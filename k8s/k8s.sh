...
function usage {
  echo -e "$1\n"
  echo "This script creates a single controlplane / multiple workers K8s cluster using *kubeadm*"
  echo "usage: k8s.sh [options]"
  echo "-v VERSION: Kubernetes version (default is 1.28.2)"
...
# Create the controlplane and workers VM using Multipass
function create-vms {
  echo "-> creating Ubuntu VMs with Multipass"

  # Creating controlplane node
  multipass launch --name controlplane --memory "$MEM" --cpus "$CPU" --disk "$DISK" || usage "Error creating [controlplane] VM"
  
  # Creating worker nodes
  if [ "$WORKERS" -ge "1" ]; then
    for i in $(seq 1 "$WORKERS"); do
      NAME=worker$i
      multipass launch --name "$NAME" --cpus "$CPU" --memory "$MEM" --disk "$DISK" || usage "Error creating [$NAME] VM"
    done
  else
...
function install_dependencies {
  # On controlplane
  echo "-> about to install dependencies on node [controlplane]"
  multipass exec controlplane -- /bin/bash -c "curl https://raw.githubusercontent.com/fleox/k8s-node/main/controlplane.sh | VERSION=$VERSION sh";
  echo "-> done installing dependencies on node [controlplane]"
  ...
  if [ "$WORKERS" -ge "1" ]; then
    for i in $(seq 1 "$WORKERS"); do
      ...
      multipass exec "$NAME" -- /bin/bash -c "curl https://raw.githubusercontent.com/fleox/k8s-node/main/worker.sh | VERSION=$VERSION sh";
...
function init_cluster {
  echo "-> initializing cluster on [controlplane]"
  # Use specific Pod CIDR if Calico CNI is used
  if [ "$CNI" == "calico" ]; then
    multipass exec controlplane -- sudo kubeadm init --v=5 --pod-network-cidr=10.100.0.0/16 --ignore-preflight-errors=NumCPU,Mem || usage "Error during cluster init"
  else
    multipass exec controlplane -- sudo kubeadm init --v=5 --ignore-preflight-errors=NumCPU,Mem || usage "Error during cluster init"
  fi
...
function get_kubeconfig {
  ...
  multipass exec controlplane -- /bin/bash -c "
    mkdir -p \$HOME/.kube
    sudo cp -i /etc/kubernetes/admin.conf \$HOME/.kube/config
    sudo chown \$(id -u):\$(id -g) \$HOME/.kube/config
  "
  # External on host
  multipass exec controlplane -- sudo cat /etc/kubernetes/admin.conf > kubeconfig.cfg || usage "Error retreiving kubeconfig"
}

# Add worker nodes
function add_nodes {
  JOIN=$(multipass exec controlplane -- /bin/bash -c "sudo kubeadm token create --print-join-command 2>/dev/null")
...
multipass exec controlplane -- /bin/bash <<EOF
...
function remove_taint {
  echo "-> Remove taint from the controlplane node"
  multipass exec controlplane -- kubectl taint nodes controlplane node-role.kubernetes.io/control-plane-
}
...
function destroy {
  ...
  if ! multipass info controlplane &>/dev/null; then
    echo "Seems there is no controlplane node here... aborting"
    exit 1
  else
    echo "-> deleting controlplane node"
    multipass delete -p controlplane
  fi
